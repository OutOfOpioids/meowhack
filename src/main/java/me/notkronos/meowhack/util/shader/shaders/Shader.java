...
public abstract class Shader {
    ...
    public Shader(String name, String path)
    {
        ...
        try (InputStream stream = Shader.class.getResourceAsStream("/shaders/" + path))
        {
            ...
            {
                assert stream != null;
                if ((read = stream.read(buffer, 0, buffer.length)) == -1) break;
        ...
        {
            Meowhack.LOGGER.error("Shader " + name + " failed to read.");
            e.printStackTrace();
            return;
        }
        ...
        {
            Meowhack.LOGGER.error("Shader " + name + "'s vertex shader failed to compile! Reason: " + glGetShaderInfoLog(vertex, 1024));
            return;
        }

        if (glGetShaderi(vertex, GL_COMPILE_STATUS) == GL11.GL_FALSE)
        {
            Meowhack.LOGGER.error("Shader " + name + "'s fragment shader failed to compile! Reason: " + glGetShaderInfoLog(fragment, 1024));
            return;
        }
        ...
        {
            Meowhack.LOGGER.error("Shader " + name + "failed to link! Reason: " + glGetProgramInfoLog(fragment, 1024));
            return;
        }
        ...
        {
            Meowhack.LOGGER.error("Shader " + name + "failed to validate! Reason: " + glGetProgramInfoLog(fragment, 1024));
            return;
        }
